...
left join {{ source('snapshot', 'dim_user') }} dim_users on dim_users.id=orders_extract_cast.id_user
left join {{ source('snapshot', 'dim_status') }} dim_status on dim_status.id_order=orders_extract_cast.id
